...
<body>
  ...
  <script>
    ...
    // Use your custom NPoint API here
    fetch("https://api.npoint.io/07b0da5498305e48d161") // replace with your own if needed
      ...
      .then(data => {
        ...
        // Populate areas when city changes
        ...
        // Load markers when area is selected
    ...
    .then(data => {
      ...
      data.forEach(p => {
        ...
        const popupContent = `
          ...
          <a href="booking.php?name=${encodeURIComponent(p.name)}&city=${encodeURIComponent(p.city)}&area=${encodeURIComponent(p.area)}&state=Maharashtra&available=${p.available_slots}&total=${p.total_slots}" style="color:blue; text-decoration:underline;">Book Now</a>
        `;

        marker.bindPopup(popupContent);

        // Optional: Redirect directly on marker click
        // marker.on('click', () => {
        //   window.location.href = `booking.html?name=${encodeURIComponent(p.name)}`;
        // });
      });
    });
}
